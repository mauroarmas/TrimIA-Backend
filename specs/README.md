...
| **005** | [El asistente sabe con quién habla](005-roles-y-areas/) | El rol y las áreas viajan hasta el asistente: reconoce cuatro interlocutores (cliente, empleado, supervisor, gerente) y les habla distinto; una persona puede ser responsable de varias áreas (N:M) y quien lo es de todas es gerente; ante baja confianza a un responsable no se le abre un caso a sí mismo sino que se le muestra qué documentos se consultaron y con cuánta cercanía; derivar a otra persona; y escribir conocimiento solo en las áreas propias. Sprint 5B. | ✅ **Backend implementado** el 2026-08-19 (43/50 tareas, 604 tests). Las 7 que faltan son la fase de panel, que por convención se enumera y se trabaja aparte. Su `research.md` **descartó** agregar un rol `GERENTE`: habría obligado a revisar 23 decoradores `@Roles(...)` —`RolesGuard` compara por igualdad exacta— y un olvido le quitaba acceso al dueño; con la tabla N:M el problema desaparece en vez de resolverse. Nace de los hallazgos del 2026-08-18 probando el panel ([docs/hallazgos-para-proxima-spec.md](../docs/hallazgos-para-proxima-spec.md)). → **Modifica a [000](000-linea-base/), [001](001-human-in-the-loop/) y [003](003-archivos-chat-conocimiento/)** (ver ahí). No toca la recuperación del RAG ni lo que alcanza un cliente. | `005-roles-y-areas` ← nace de `004-chat-tiempo-real` |
...
2. **La fase final de panel de cada spec vive en otro repositorio.** Toda spec que
   ...
   Las de 003 y 004 ya están hechas allá (commits `b256cd5`…`b684974`); las de **005
   están pendientes** (`specs/005-roles-y-areas/tasks.md` §Fase 9, T044-T050: la
   pantalla de empleados **no existe** todavía). La nota de `plan_de_trabajo.md` §8 que
   dice que el panel todavía no consume el 5A quedó desactualizada.
